...
class ContactBook:
    def __init__(self, root):
        ...
        self.frame.pack(padx=0, pady=20)
        ...
        self.button_search.grid(row=9, column=0, columnspan=2, padx=5, pady=5)

        self.label_delete = tk.Label(self.frame, text="Delete:")
        self.label_delete.grid(row=8, column=0, padx=5, pady=5)
        self.entry_delete = tk.Entry(self.frame)
        self.entry_delete.grid(row=8, column=1, padx=5, pady=5)
    ...
    def add_contact(self):
        ...
        if name and phone:
            ...
            self.c.execute("INSERT INTO contacts (name, phone, email, age, place) VALUES (?, ?, ?, ?, ?)", (name, phone, email, age, place))
            self.conn.commit()
    ...
    def display_contacts(self):
        ...
        if contacts:
            contact_list = "\n".join([f"Name: {contact[0]}, Phone: {contact[1]}, Email: {contact[2]}, Age: {contact[3]}, Place: {contact[4]}" for contact in contacts])
    ...
    def search_contact(self):
        ...
        if search_name:
            self.c.execute("SELECT * FROM contacts WHERE name=?", (search_name,))
            contact = self.c.fetchone()
            if contact:
                messagebox.showinfo("Contact Details", f"Name: {contact[0]}\nPhone: {contact[1]}\nEmail: {contact[2]}\nAge:{contact[3]}\nPlace: {contact[4]}")
            else:
                messagebox.showinfo("Contact Details", "Contact not found.")
        else:
            messagebox.showerror("Error", "Please enter a name to search.")

    def delete_contact(self):
        name_to_delete = self.entry_delete.get()
        if name_to_delete:
